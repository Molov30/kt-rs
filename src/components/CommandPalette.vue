...
const flatFiltered = computed(() => {
  ...
  for (const group of props.groups) {
    for (const command of group.commands) {
      if (!q || command.label.toLowerCase().includes(q) || command.description?.toLowerCase().includes(q)) {
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        <!-- palette -->
        <div
          class="relative w-full max-w-lg rounded-xl bg-zinc-900 border border-zinc-800 shadow-2xl overflow-hidden"
        >
          <!-- config view -->
          ...
          <template v-else>
            ...
            <div class="flex items-center gap-3 px-4 py-3 border-b border-zinc-800">
              ...
              <input
                ...
                class="flex-1 bg-transparent text-zinc-100 text-sm placeholder:text-zinc-600 focus:outline-none"
                @keydown="onKeydown"
              />
            ...
            <div class="max-h-80 overflow-y-auto py-1.5">
              <template v-if="filteredGroups.length === 0">
                <p class="text-sm text-zinc-600 text-center py-8">No results for "{{ query }}"</p>
              </template>
              ...
              <template v-for="group in filteredGroups" :key="group.id">
                ...
                <button
                  ...
                  :key="cmd.id"
                  class="w-full flex items-center justify-between gap-3 px-4 py-2 text-left transition-colors"
                  :class="isActive(cmd) ? 'bg-zinc-800' : 'hover:bg-zinc-800/50'"
                  @click="selectCommand(cmd)"
                  @mouseenter="activeIndex = indexOfCommand(cmd)"
                >
                  <CommandIcon v-if="cmd.icon" :icon="cmd.icon" />
                  <div
                    v-else
                    class="w-4 h-4 shrink-0 rounded bg-zinc-800 border border-zinc-700 flex items-center justify-center"
                  >
                    <span class="text-[9px] text-zinc-600 font-mono leading-none">
                      {{ cmd.label.charAt(0).toUpperCase() }}
                    </span>
                  </div>

                  <div class="flex flex-col min-w-0 flex-1">
                    <span class="text-sm text-zinc-200 truncate">{{ cmd.label }}</span>
                    <span v-if="cmd.description" class="text-xs text-zinc-500 truncate">{{ cmd.description }}</span>
                  </div>

                  <div class="flex items-center gap-2 shrink-0">
                    <span v-if="cmd.params?.length" class="text-[10px] text-zinc-600 border border-zinc-700 rounded px-1.5 py-0.5">config</span>
                    <kbd v-if="cmd.shortcut" class="text-[10px] text-zinc-500 border border-zinc-700 rounded px-1.5 py-0.5">{{ cmd.shortcut }}</kbd>
                  </div>
                </button>

                <div class="mx-4 my-1 border-b border-zinc-800/60 last:hidden" />
              </template>
            </div>

            <!-- footer -->
            <div class="flex items-center gap-3 px-4 py-2 border-t border-zinc-800 text-[10px] text-zinc-600">
