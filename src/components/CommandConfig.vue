...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center gap-2 border-b border-zinc-800 pb-3">
      ...
      <span class="text-zinc-200 text-sm font-medium">{{ command.label }}</span>
    </div>

    <div class="flex flex-col gap-3">
      <div v-for="param in command.params" :key="param.name" class="flex flex-col gap-1">
        <label class="text-xs text-zinc-500 font-medium uppercase tracking-wider">
          {{ param.label }}
        </label>
